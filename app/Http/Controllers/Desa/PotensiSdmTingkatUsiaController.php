...
function newTingkatUsia(){
	$route = array("main" => "potensi", "sub" => "sdm", "title" => "Potensi - Tingkat Usia");
        return view('desa.potensi.new-tingkat-usia', array("route" => $route));
}

function insertTingkatUsia (Request $request) {
...
function updateTingkatUsia (Request $request) {
...
function deleteTingkatUsia (Request $request) {
$id=Crypt::decrypt($request->input('id'));
$record = TingkatUsia::find($id);
if($record){
$record->delete();
